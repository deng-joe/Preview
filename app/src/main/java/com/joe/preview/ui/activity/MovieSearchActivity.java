...
import com.joe.preview.adapter.MovieSearchListAdapter;
...
public class MovieSearchActivity extends BaseActivity implements SearchView.OnQueryTextListener,
    ...
    private void initializeView() {
        ...
        movieSearchListAdapter = new MovieSearchListAdapter(this);
        binding.includedLayout.moviesList.setLayoutManager(new LinearLayoutManager(getApplicationContext(),
                LinearLayoutManager.HORIZONTAL, false));
        binding.includedLayout.moviesList.setAdapter(movieSearchListAdapter);

        SnapHelper startSnapHelper = new PagerSnapHelper(position -> {
            Movie trailer = movieSearchListAdapter.getItem(position);
    ...
    private void initializeViewModel() {
        movieSearchViewModel = ViewModelProviders.of(this, viewModelFactory).get(MovieSearchViewModel.class);
    }
    ...
    private void querySearch(String text) {
        movieSearchViewModel.searchMovie(text);
        movieSearchViewModel.getMoviesLiveData().observe(this, listResource -> {
    ...
    private void handleSuccessResponse(List<Movie> movies) {
        hideLoader();
        ...
        movieSearchListAdapter.setItems(movies);
        new Handler().postDelayed(() -> {
            if (movieSearchListAdapter.getItemCount() > 0)
                updateBackground(movieSearchListAdapter.getItem(0).getPosterPath());
        }, 400);
    }
    ...
    public void onItemClick(View parentView, View childView, int position) {
        movieSearchViewModel.getMoviesLiveData().removeObservers(this);
        ActivityOptionsCompat optionsCompat = ActivityOptionsCompat.makeSceneTransitionAnimation(this,
                new Pair(childView.findViewById(R.id.image), TRANSITION_IMAGE_NAME));
        NavigationUtil.redirectToMovieDetails(this, movieSearchListAdapter.getItem(position), optionsCompat);
    }
